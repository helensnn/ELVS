...
class EffNetAttention(nn.Module):
    def __init__(self, label_dim=527, b=0, pretrain=True, head_num=4, load_model_path=None):
        ...
        if pretrain == False:
            ...
            self.effnet = EfficientNet.from_pretrained('efficientnet-b'+str(b),weights_path=load_model_path, in_channels=1)
    ...
    def forward(self, x, nframes=1056):
        ...
        x = self.avgpool(x) # Frequency Mean Pooling in: (b, feature, 128, 1024) out: (b, feature, 32, 1024)
        x = x.transpose(2,3) # (b, feature, 1024, 32)
        out, norm_att = self.attention(x)
        return out
